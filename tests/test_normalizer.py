...
from normalizer import (normalize, _valid_ip, _valid_domain, _valid_hash,
...
class TestNormalize:
    def _r(self, value, ioc_type="ip", source="feed_a", severity="High", desc=""):
    ...
    def test_valid_public_ip_passes(self):
        assert any(r["value"] == "185.220.101.45" for r in normalize([self._r("185.220.101.45")]))
    def test_private_ip_filtered(self):
        assert not any(r["value"] == "192.168.1.1" for r in normalize([self._r("192.168.1.1")]))
    def test_whitelisted_domain_filtered(self):
        assert not any(r["value"] == "google.com" for r in normalize([self._r("google.com", "domain")]))
    def test_within_feed_deduplication(self):
        raw = [self._r("185.220.101.45"), self._r("185.220.101.45")]
        assert len([r for r in normalize(raw) if r["value"] == "185.220.101.45"]) == 1
    def test_cross_feed_not_deduplicated(self):
        raw = [self._r("185.220.101.45", source="feed_a"), self._r("185.220.101.45", source="feed_b")]
        assert len([r for r in normalize(raw) if r["value"] == "185.220.101.45"]) == 2
    def test_severity_normalised(self):
        assert normalize([self._r("185.220.101.45", severity="critical")])[0]["severity"] == "Critical"
    def test_empty_value_filtered(self):
        assert normalize([{"value": "", "type": "ip", "source": "feed", "severity": "Low", "description": ""}]) == []
    def test_output_has_required_fields(self):
        result = normalize([self._r("185.220.101.45")])
        assert {"value", "type", "source", "severity", "category", "first_seen"}.issubset(result[0].keys())
    def test_hash_subtype_md5(self):
        assert normalize([self._r("44d88612fea8a8f36de82e1278abb02f", "hash")])[0]["hash_subtype"] == "md5"
    def test_hash_subtype_sha256(self):
        h = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
        assert normalize([self._r(h, "hash")])[0]["hash_subtype"] == "sha256"
    def test_category_enriched(self):
        assert normalize([self._r("lockbit-stage.tk", "domain", desc="ransomware stage")])[0]["category"] == "ransomware"
